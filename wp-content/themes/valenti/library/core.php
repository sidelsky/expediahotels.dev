...
if ( ! function_exists( 'cb_byline' ) ) {
    function cb_byline( $cb_cat = true, $cb_post_id = NULL, $cb_short_comment_line = false, $cb_posts_on = false, $cb_post_views_off = false, $args = array() ) {
        ...
        $cb_author = '<div class="cb-author cb-byline-element' . esc_attr( $author_class ) . '">';
        if ( function_exists( 'coauthors_posts_links' ) ) {
            $cb_author .= '<i class="fa fa-user"></i> ' . coauthors_posts_links( null, null, null, null, false );
        } else {
           $cb_author .= '<i class="fa fa-user"></i> <a href="' . get_author_posts_url( cb_get_author_by_post_id( $cb_post_id ) ) . '" class="fn">' . get_the_author_meta( 'display_name', cb_get_author_by_post_id( $cb_post_id ) ) . '</a>';
        }
        $cb_author .= '</div>';

        $cb_date = ' <div class="cb-date cb-byline-element"><i class="fa fa-clock-o"></i> <time class="' . esc_attr( $updated_class ) . '" datetime="' . get_the_time('Y-m-d', $cb_post_id) . '">' . date_i18n( get_option('date_format'), strtotime(get_the_time("Y-m-d", $cb_post_id )) ) . '</time></div>';
...
if ( ! function_exists( 'cb_author_details' ) ) {
    function cb_author_details($cb_author_id, $cb_desc = true) {
        ...
        if ( $cb_author_go != NULL ) { $cb_author_output .= ' <a href="' . esc_url( $cb_author_go ) . '" rel="publisher" target="_blank" data-cb-tip="Google+" class="cb-googleplus cb-tip-bot" ><i class="fa fa-google-plus"></i></a>'; }
        if ( $cb_author_in != NULL ) { $cb_author_output .= ' <a href="' . esc_url( $cb_author_in ) . '" target="_blank" data-cb-tip="Instagram" class="cb-instagram cb-tip-bot" ><i class="fa fa-instagram"></i></a>'; }
...
if ( ! function_exists( 'cb_author_box' ) ) {
    function cb_author_box( $post, $cb_author_id_sc = NULL, $cb_block_title = NULL ) {
        ...
        $cb_author_output .= '<div class="cb-author-title"><a href="' . get_author_posts_url( $cb_author_id ) . '"><span>' . $cb_author_name . '</span></a></div>';
        ...
        if ( $cb_author_www != NULL ) { $cb_author_output .= ' <a href="' . esc_url( $cb_author_www ) . '" target="_blank"><i class="fa fa-link cb-tip-bot" data-cb-tip="'.__('Website', 'cubell') . '"></i></a> '; }
        if ( $cb_author_tw != NULL ) { $cb_author_output .= ' <a href="//www.twitter.com/' . $cb_author_tw. '" target="_blank" ><i class="fa fa-twitter cb-tip-bot" data-cb-tip="Twitter"></i></a>'; }
        if ( $cb_author_go != NULL ) { $cb_author_output .= ' <a href="' . esc_url( $cb_author_go ) . '?rel=author" rel="publisher" target="_blank" data-cb-tip="Google+" class="cb-googleplus cb-tip-bot" ><i class="fa fa-google-plus"></i></a>'; }
    ...
    function cb_background_image() {
        ...
        if ( ( $cb_bg_to == 'off' ) || ( ( $cb_bg_to == 'only-hp' ) && ( ! is_front_page() ) ) ) {
            ...
            if ( is_singular() ) {
                ...
                if ( $cb_featured_image_style != 'full-background' && $cb_featured_image_style != 'parallax' )  {
...
if ( ! function_exists( 'cb_featured_image' ) ) {
    function cb_featured_image( $post, $cb_style ) {
        ...
        if ( ( $cb_style == 'parallax' ) && ( $cb_is_mobile == true ) ) {
            $cb_header .= '<div id="cb-fis-wrap" class="cb-entry-header hentry cb-fis cb-style-full-background">';
        } else {
            $cb_header .= '<div id="cb-fis-wrap" class="cb-entry-header hentry cb-fis cb-style-' . $cb_style. '">';
        }
        ...
        if ( $cb_style != 'page' ) {
            ...
            if ( $cb_meta_onoff == 'on' ) {
                $cb_header .= cb_byline(true, $cb_post_id, false, true, false, array( 'schema' => true ) );
            }
        }
        ...
        if ( $cb_style == 'off' ) {
            ...
            if ( has_post_thumbnail() ) {
                ...
                } elseif ( ( $cb_page_featured_style == '5' ) || ( ( $cb_page_featured_style == '4' ) &&  ( $cb_is_mobile == true )  ) ) {
