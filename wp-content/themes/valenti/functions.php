...
require_once get_template_directory() . '/inc/core/woocommerce.php';
...
if ( ! function_exists( 'cb_scripts_and_styles' ) ) {
    function cb_scripts_and_styles() {
      if (!is_admin()) {
        ...
        wp_enqueue_style( 'cb-main-stylesheet',  get_template_directory_uri() . '/library/css/' . $cb_style_name . '.css', array(), CB_VER, 'all' );
        // Register fonts
        $cb_font = cb_fonts();
        ...
        wp_enqueue_script( 'cb-js-ext',  get_template_directory_uri() . '/library/js/jquery.ext.js', array( 'jquery' ), CB_VER, true);
        ...
        if ( ot_get_option('cb_minify_js_onoff', 'on') != 'off' ) {
            wp_enqueue_script( 'cb-js',  get_template_directory_uri()  . '/library/js/cb-scripts.min.js', array( 'jquery', 'cb-js-ext', 'jquery-ui-tabs'   ), CB_VER, true);
        } else {
            wp_enqueue_script( 'cb-js',  get_template_directory_uri()  . '/library/js/cb-scripts.js', array( 'jquery', 'cb-js-ext', 'jquery-ui-tabs'  ), CB_VER, true);
        }
...
if ( ! function_exists( 'cb_post_admin_scripts_and_styles' ) ) {
    function cb_post_admin_scripts_and_styles($hook) {
        ...
        if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'edit-tags.php' || $hook == 'profile.php' || $hook == 'appearance_page_ot-theme-options' || $hook == 'user-edit.php' || $hook == 'appearance_page_radium_demo_installer' ||  $hook == 'edit-tags.php' || $hook == 'term.php' || $hook == 'widgets.php' ) {
            wp_enqueue_style('farbtastic');
            wp_enqueue_script('farbtastic');
            wp_enqueue_script( 'admin-js',  get_template_directory_uri()  . '/library/admin/js/cb-admin.js', array(), CB_VER, true);
            wp_enqueue_style('fontawesome', get_template_directory_uri() . '/library/css/fontawesome/css/font-awesome.min.css', array(), '4.6.3', 'all' );
